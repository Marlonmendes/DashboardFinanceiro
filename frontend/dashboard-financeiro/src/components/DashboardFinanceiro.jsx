...
import { ArrowUpRight, ArrowDownLeft, Search, UtensilsCrossed,
...
/**
 * Tela Dashboard — agora SEM sidebar embutida.
 * A sidebar é renderizada uma única vez pelo App.jsx, fora desta tela,
 * para que ela não recarregue/perca estado ao navegar entre telas.
 *
 * Props:
 * - isMobile: booleano controlado pelo App.jsx (mesmo listener de resize)
 */
export default function Dashboard({ isMobile = false }) {
  ...
  const fetchDados = async () => {
    try {
      ...
      if (Array.isArray(dashboardData?.porMes)) {
        setAnalyticsData(dashboardData?.porMes);
      } else {
        setAnalyticsData([]);
      }
    } catch (err) {
